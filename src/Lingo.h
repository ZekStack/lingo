...
struct LingoEnumTraits {
	static constexpr bool valid = false;
};

template<typename T>
struct LingoEnumTraits<T, true> {
	using Underlying = std::underlying_type_t<T>;
	static constexpr bool valid = std::is_unsigned_v<Underlying> && sizeof(Underlying) <= sizeof(uint16_t);
};

template<typename T>
inline constexpr bool isLingoEnum = LingoEnumTraits<T>::valid;

template<typename T>
struct LingoDomainToken {
	static inline uint8_t value = 0;
};

template<typename T>
const void *domainToken() {
...
template<typename T>
constexpr uint16_t enumValue(T value) {
	static_assert(isLingoEnum<T>, "Lingo enums must use an unsigned underlying type up to 16 bits");
...
class LingoLanguage {
  public:
	...
	template<typename TLanguage>
	constexpr LingoLanguage &operator=(TLanguage language) {
...
template<typename TKey>
LingoEntry(TKey, const char *) -> LingoEntry<TKey>;
...
LingoTable(LingoEntry<TKey>, TRest...)
    -> LingoTable<TKey, sizeof...(TRest) + 1>;
